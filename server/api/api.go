...
// This package is for serving the actual cerberusd API.
...
func corsValidator() OriginValidator {
	...
	cerberusRegex := regexp.MustCompile(`^https://([[:alnum:]\-_]+\.)*cerberus\.uraanai\.com$`)

	// *.cerberusiovpjcahpzkrewelclulmszwbqpzmzgub37gbcjlvluxtruqad.onion
	cerberusOnionRegex := regexp.MustCompile(`^https?://([[:alnum:]\-_]+\.)*cerberusiovpjcahpzkrewelclulmszwbqpzmzgub37gbcjlvluxtruqad\.onion$`)
	...
	v := func(origin string) bool {
		if cerberusRegex.MatchString(origin) {
			return true
		}

		if cerberusOnionRegex.MatchString(origin) {
			return true
		}
