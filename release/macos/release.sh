...
INSTALLER=cerberus-bridge-$VERSION.pkg

mkdir -p flat-uninstall/uninstall.pkg/payload
mkdir -p flat-install/install.pkg/payload/Applications/Utilities/CERBERUS\ Bridge
...
# second, make installer and add cerberusd and uninstaller
...
if [ -r $SIGN_APP_PASSPHRASE_F ]; then
    rcodesign sign --p12-file $SIGN_APP_CERT --p12-password-file $SIGN_APP_PASSPHRASE_F --code-signature-flags runtime /release/build/cerberusd
fi

cp /release/build/cerberusd Applications/Utilities/CERBERUS\ Bridge/
cp ../../../uninstall.pkg Applications/Utilities/CERBERUS\ Bridge/
