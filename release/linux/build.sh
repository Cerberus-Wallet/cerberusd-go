...
git config --global --add safe.directory /cerberusd

case $CERBERUSD_BUILD in
  "go-arm64-musl")
    CGO_ENABLED=1 CC="/usr/local/musl/bin/aarch64-unknown-linux-musl-gcc" GOARCH=arm64 \
        ...
        -o release/linux/build/cerberusd-linux-arm64
    ;;
  ...
  "go-386-musl")
     CGO_ENABLED=1 CC="/usr/local/musl/bin/i686-unknown-linux-musl-gcc" GOARCH=386 \
        ...
        -o release/linux/build/cerberusd-linux-386
    ;;
  ...
  "go-amd64-musl")
     CGO_ENABLED=1 CC="/usr/local/musl/bin/x86_64-unknown-linux-musl-gcc" GOARCH=amd64 \
        ...
        -o release/linux/build/cerberusd-linux-amd64
    ;;
